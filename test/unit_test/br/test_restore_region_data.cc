...
static void TestImportDataToRegion(const BrRestoreRegionDataTest& br_restore_region_data_test,
  ...
  for (const auto& [region_id, region] : id_group_map) {
    auto restore_region_data = std::make_shared<br::RestoreRegionData>(
        ...
        br_restore_region_data_test.storage, br_restore_region_data_test.storage_internal, id_group_map[region_id],
        cf_file_name, group_belongs_to_whom, br_restore_region_data_test.restore_region_timeout_s);
...
TEST_F(BrRestoreRegionDataTest, TestDocumentRegionSdkData) {
  ...
  const std::string& region_file_name = "document_region_sdk_data.sst";
  const std::string region_file_path = base_dir + region_file_name;
  ...
  for (const auto& [region_id, region_ptr] : kvs) {
    ...
    auto ret = region->ParseFromString(region_ptr);
    EXPECT_TRUE(ret);
    id_region_map[region->id()] = region;
  }

  const std::string& cf_file_name = "document_cf_sst_meta_sdk_data.sst";
  const std::string cf_file_path = base_dir + cf_file_name;

  reader_sst = std::make_shared<br::SstFileReader>();

  kvs.clear();
  status = reader_sst->ReadFile(region_file_path, kvs);
  EXPECT_EQ(status.error_code(), dingodb::pb::error::OK);

  std::map<int64_t, std::shared_ptr<dingodb::pb::common::BackupDataFileValueSstMetaGroup>> id_group_map;

  for (const auto& [region_id, group_ptr] : kvs) {
    auto group = std::make_shared<dingodb::pb::common::BackupDataFileValueSstMetaGroup>();
    auto ret = group->ParseFromString(group_ptr);
    EXPECT_TRUE(ret);
    id_group_map[std::stoll(region_id)] = group;
  }

  for (const auto& [region_id, region] : id_group_map) {
    auto restore_region_data = std::make_shared<br::RestoreRegionData>(
        coordinator_interaction, interaction, id_region_map[region_id], restorets, restoretso_internal, storage,
        storage_internal, id_group_map[region_id], backup_meta_region_cf_name, "group_belongs_to_whom",
        restore_region_timeout_s);
  ...
  TestImportDataToRegion(*this, file_name);
}



TEST_F(BrRestoreRegionDataTest, TestDocumentRegionSqlData) {
  const std::string& file_name = "document_region_sql_data.sst";
  TestImportDataToRegion(*this, file_name);
}

TEST_F(BrRestoreRegionDataTest, TestIndexRegionSdkData) {
  const std::string& file_name = "index_region_sdk_data.sst";
  TestImportDataToRegion(*this, file_name);
}

TEST_F(BrRestoreRegionDataTest, TestIndexRegionSqlData) {
  const std::string& file_name = "index_region_sql_data.sst";
  TestImportDataToRegion(*this, file_name);
}

TEST_F(BrRestoreRegionDataTest, TestStoreRegionSdkData) {

...
DEFINE_uint32(vector_index_flat_smid_test_dimension, 512, "vector index flat smid test dimension. default 512");
DEFINE_uint32(vector_index_flat_smid_test_data_base_size, 1000000,
              "vector_index_flat_smid_test_data_base_size. default 1000000");

namespace dingodb {

class VectorIndexFlatSmidTest : public testing::Test {
 protected:
  static void SetUpTestSuite() {
    vector_index_thread_pool = std::make_shared<ThreadPool>("vector_index", 4);
    dimension = FLAGS_vector_index_flat_smid_test_dimension;
    data_base_size = FLAGS_vector_index_flat_smid_test_data_base_size;
  }
  ...
  inline static faiss::idx_t dimension = FLAGS_vector_index_flat_smid_test_dimension;
  inline static int data_base_size = FLAGS_vector_index_flat_smid_test_data_base_size;
  inline static std::vector<float> data_base;
  inline static ThreadPoolPtr vector_index_thread_pool;
};

TEST_F(VectorIndexFlatSmidTest, Create) {
...
TEST_F(VectorIndexFlatSmidTest, Add) {
  butil::Status ok;
...
TEST_F(VectorIndexFlatSmidTest, Search) {
  butil::Status ok;
  ...
  auto lambda_search_function = [&lambda_time_now_function, &lambda_time_diff_microseconds_function,
    ...
    LOG(INFO) << fmt::format("{} search {} dimension: {} total : {} topk : {:<3} n : {:<3} time cost : {} (ms)",

  ...
  for (auto &tenant : tenants) {
    ...
    for (const auto &schema : schemas) {
      ...
      for (const auto &table_definition_with_id : table_definition_with_ids) {
        tables_and_indexes.add_tables()->CopyFrom(table_definition_with_id.table_definition());
      }
      ...
      for (const auto &index_definition_with_id : index_definition_with_ids) {
        tables_and_indexes.add_indexes()->CopyFrom(index_definition_with_id.index_definition());
      }

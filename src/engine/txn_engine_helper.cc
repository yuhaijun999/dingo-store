...
butil::Status TxnEngineHelper::BackupData(std::shared_ptr<Context> ctx, RawEnginePtr raw_engine,
  ...
  // txn
  std::vector<pb::common::KeyValue> kv_data;
  std::vector<pb::common::KeyValue> kv_write;

  // non txn
  std::vector<pb::common::KeyValue> kv_default;
  std::vector<pb::common::KeyValue> kv_scalar;
  std::vector<pb::common::KeyValue> kv_table;
  std::vector<pb::common::KeyValue> kv_scalar_speedup;
...
butil::Status TxnEngineHelper::DoBackupDataCoreTxn(std::shared_ptr<Context> ctx, RawEnginePtr raw_engine,
                                                   store::RegionPtr region, const pb::common::RegionType &region_type,
                                                   int64_t backup_tso, std::vector<pb::common::KeyValue> &kv_data,
                                                   std::vector<pb::common::KeyValue> &kv_write) {
  int64_t start_time_ms = Helper::TimestampMs();
  int64_t end_time_ms = 0;
  int64_t total_iter_count = 0;
...
butil::Status TxnEngineHelper::DoBackupDataCoreNonTxn(std::shared_ptr<Context> ctx, RawEnginePtr raw_engine,
  ...
  auto lambda_emplace_back_function = [&region, &prefix, &region_part_id, &kv_default, &kv_scalar, &kv_table,
    ...
    {
      pb::common::KeyValue kv;
      kv.set_key(std::string(default_iter_key));
      kv.set_value(std::string(default_iter_value));
      kv_default.push_back(std::move(kv));
    }
    ...
    if (type == pb::common::RegionType::INDEX_REGION) {
      ...
      if (status.ok()) {
        pb::common::KeyValue kv;
        kv.set_key(scalar_key);
        kv.set_value(scalar_value);
        kv_scalar.push_back(std::move(kv));
      }
      ...
      if (status.ok()) {
        pb::common::KeyValue kv;
        kv.set_key(table_key);
        kv.set_value(table_value);
        kv_table.push_back(std::move(kv));
      }
      ...
      for (const auto &fields : scalar_schema.fields()) {
        if (fields.enable_speed_up()) {
          ...
          if (status.ok()) {
            pb::common::KeyValue kv;
            kv.set_key(scalar_speedup_key);
            kv.set_value(scalar_speedup_value);
            kv_scalar_speedup.push_back(std::move(kv));
          }
        }
      }
    }
...
butil::Status TxnEngineHelper::DoBackupDataForDocumentNonTxn(
    ...
    const pb::common::RegionType &region_type, int64_t backup_tso, std::vector<pb::common::KeyValue> kv_default,
    std::vector<pb::common::KeyValue> kv_scalar, std::vector<pb::common::KeyValue> kv_table,
    std::vector<pb::common::KeyValue> kv_scalar_speedup) {
  return DoBackupDataCoreNonTxn(ctx, raw_engine, region, region_type, backup_tso, kv_default, kv_scalar, kv_table,
                                kv_scalar_speedup);
}
...
butil::Status TxnEngineHelper::DoWriteDataAndCheckForTxn(
    ...
    std::vector<pb::common::KeyValue> &kv_data, std::vector<pb::common::KeyValue> &kv_write) {
  ...
  {
    pb::common::KeyValue kv;
    kv.set_key(std::string(write_iter_key));
    kv.set_value(std::string(write_iter_value));
    kv_write.push_back(std::move(kv));
  }
...
butil::Status TxnEngineHelper::WriteSstFileForTxn(
    ...
    const std::string &region_type_name, const std::vector<pb::common::KeyValue> &kv_data,
    const std::vector<pb::common::KeyValue> &kv_write, pb::common::BackupDataFileValueSstMetaGroup *sst_meta_group) {
  butil::Status status;
...
butil::Status TxnEngineHelper::WriteSstFileForNonTxn(
    store::RegionPtr region, int64_t instance_id, const pb::common::RegionType &region_type,
    const pb::common::StorageBackend &storage_backend, const std::string &backup_file_prefix,
    const std::string &region_type_name, const std::vector<pb::common::KeyValue> &kv_default,
    const std::vector<pb::common::KeyValue> &kv_scalar, const std::vector<pb::common::KeyValue> &kv_table,
    const std::vector<pb::common::KeyValue> &kv_scalar_speedup,

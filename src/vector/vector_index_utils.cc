...
butil::Status VectorIndexUtils::ValidateVectorWithDocumentSpeedupJsonParameter(
  ...
  for (const auto& field : scalar_schema.fields()) {
    ...
    switch (field.field_type()) {
      case pb::common::ScalarFieldType::BYTES:
        // unsupport bytes speed up for now
        ...
        //   std::string error_msg =
        //       fmt::format("json_parameter is not consistent with scalar_schema, field_name:{}, field_type:BYTES vs
        //       {}",
        //                   field.key(),
        //                   DocumentCodec::GetTokenizerTypeString(column_tokenizer_parameter[field.key()]));
        //   DINGO_LOG(ERROR) << error_msg;
        //   return butil::Status(pb::error::Errno::EILLEGAL_PARAMTETERS, error_msg);
        // }
        {
          std::string error_msg =
              fmt::format("json_parameter is not support pb::common::ScalarFieldType::BYTES  key: {} ", field.key());
          DINGO_LOG(ERROR) << error_msg;
          return butil::Status(pb::error::Errno::EILLEGAL_PARAMTETERS, error_msg);
        }
        break;

...
BackupSdkMeta::BackupSdkMeta(ServerInteractionPtr coordinator_interaction, std::string storage_internal)
...
butil::Status BackupSdkMeta::Backup() {
  ...
  kv.set_key("SDK_META");
  kv.set_value(meta_all_->SerializeAsString());
  kvs.push_back(std::move(kv));

  rocksdb::Options options;
  std::shared_ptr<SstFileWriter> sst = std::make_shared<SstFileWriter>(options);

  std::string file_path = storage_internal_ + "/" + dingodb::Constant::kCoordinatorSdkMetaSstName;
  ...
  backup_sdk_meta_->set_file_name(dingodb::Constant::kCoordinatorSdkMetaSstName);

  return butil::Status::OK();
}

std::shared_ptr<dingodb::pb::common::BackupMeta> BackupSdkMeta::GetBackupSdkMeta() { return backup_sdk_meta_; }

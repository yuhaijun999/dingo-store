...
butil::Status RestoreMeta::CreateAutoIncrementsToMeta() {
  if (table_increment_group_ && table_increment_group_->table_increments_size() > 0) {
    ...
    auto status = coordinator_interaction_->SendRequest("MetaService", "CreateAutoIncrements", request, response);
    if (!status.ok()) {
      DINGO_LOG(ERROR) << status.error_cstr();
      return status;
    }
    ...
    DINGO_LOG(WARNING) << "table_increment_group is empty, skip create auto increments.";
  }

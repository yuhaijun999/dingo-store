...
butil::Status Backup::Run() {
  ...
  // get executor txn meta
  ::google::protobuf::RepeatedPtrField<dingodb::pb::coordinator::ScanRegionInfo> scan_region_infos;
  status = GetSqlMetaRegion(br::InteractionManager::GetInstance().GetCoordinatorInteraction(), scan_region_infos);
  if (!status.ok()) {
    DINGO_LOG(ERROR) << status.error_cstr();
    return status;
  }
...
butil::Status Backup::GetSqlMetaRegion(
  ...
  auto status = coordinator_interaction->SendRequest("CoordinatorService", "ScanRegions", request, response);
  if (!status.ok()) {
    return status;
  }
  ...
  scan_region_infos.Add(response.regions().begin(), response.regions().end());
